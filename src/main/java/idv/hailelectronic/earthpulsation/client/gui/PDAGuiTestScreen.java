...
import idv.hailelectronic.earthpulsation.network.PDAGuiTestButtonMessage;
import idv.hailelectronic.earthpulsation.EarthPulsationMod;

import com.mojang.blaze3d.vertex.PoseStack;
import com.mojang.blaze3d.systems.RenderSystem;

public class PDAGuiTestScreen extends AbstractContainerScreen<PDAGuiTestMenu> {
	private final Level world;
	private final int x, y, z;
	private final Player entity;
	Checkbox H;

	public PDAGuiTestScreen(PDAGuiTestMenu container, Inventory inventory, Component text) {
	...
	public void init() {
		...
		this.addRenderableWidget(new Button(this.leftPos + 167, this.topPos + 0, 30, 20, new TextComponent("Close"), e -> {
			if (true) {
				EarthPulsationMod.PACKET_HANDLER.sendToServer(new PDAGuiTestButtonMessage(0, x, y, z));
				PDAGuiTestButtonMessage.handleButtonAction(entity, 0, x, y, z);
			}
		}));
		H = new Checkbox(this.leftPos + 69, this.topPos + 203, 150, 20, new TextComponent(""), false);
		PDAGuiTestMenu.guistate.put("checkbox:H", H);
