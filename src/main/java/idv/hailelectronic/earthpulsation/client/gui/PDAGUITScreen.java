...
import idv.hailelectronic.earthpulsation.network.PDAGUITButtonMessage;
import idv.hailelectronic.earthpulsation.EarthPulsationMod;

import com.mojang.blaze3d.vertex.PoseStack;
import com.mojang.blaze3d.systems.RenderSystem;

public class PDAGUITScreen extends AbstractContainerScreen<PDAGUITMenu> {
	private final Level world;
	private final int x, y, z;
	private final Player entity;
	Checkbox H;

	public PDAGUITScreen(PDAGUITMenu container, Inventory inventory, Component text) {
	...
	public void init() {
		...
		this.addRenderableWidget(new Button(this.leftPos + 167, this.topPos + 0, 30, 20, new TextComponent("Close"), e -> {
			if (true) {
				EarthPulsationMod.PACKET_HANDLER.sendToServer(new PDAGUITButtonMessage(0, x, y, z));
				PDAGUITButtonMessage.handleButtonAction(entity, 0, x, y, z);
			}
		}));
		...
		PDAGUITMenu.guistate.put("checkbox:H", H);
		...
		this.addRenderableWidget(new Button(this.leftPos + 15, this.topPos + 32, 70, 20, new TextComponent("Test page"), e -> {
			if (true) {
				EarthPulsationMod.PACKET_HANDLER.sendToServer(new PDAGUITButtonMessage(2, x, y, z));
				PDAGUITButtonMessage.handleButtonAction(entity, 2, x, y, z);
			}
		}));
	}
}
