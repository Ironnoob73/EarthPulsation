...
public class PlaceOnWaterPlateBlock extends EarthPulsationModElements.ModElement {
		...
		public void registerFeature(RegistryEvent.Register<Feature<?>> event) {
			...
			configuredFeature = feature.withConfiguration(
					(new BlockClusterFeatureConfig.Builder(new SimpleBlockStateProvider(block.getDefaultState()), new SimpleBlockPlacer())).tries(64)
							.build())
					.withPlacement(Placement.COUNT_NOISE.configure(new NoiseDependant(-0.8, 0, 0)));
			event.getRegistry().register(feature.setRegistryName("place_on_water_plate"));
			Registry.register(WorldGenRegistries.CONFIGURED_FEATURE, new ResourceLocation("earth_pulsation:place_on_water_plate"), configuredFeature);
		}
	}
	@SubscribeEvent
	...
	public static class BlockCustomFlower extends FlowerBlock {
		public BlockCustomFlower() {
			super(Effects.SPEED, 5, Block.Properties.create(Material.PLANTS).doesNotBlockMovement().sound(SoundType.LILY_PADS)
					.hardnessAndResistance(0f, 0f).setLightLevel(s -> 0));
			setRegistryName("place_on_water_plate");
		}
