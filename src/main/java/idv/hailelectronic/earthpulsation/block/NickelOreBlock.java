...
public class NickelOreBlock extends EarthPulsationModElements.ModElement {
	...
	public static class CustomBlock extends Block {
		public CustomBlock() {
			super(Block.Properties.create(Material.ROCK).sound(SoundType.METAL).hardnessAndResistance(10f, 20f).setLightLevel(s -> 0).harvestLevel(2)
	...
	private static class CustomRuleTest extends RuleTest {
		...
		public boolean test(BlockState blockAt, Random random) {
			...
			if (blockAt.getBlock() == YellowStoneBlock.block)
				blockCriteria = true;
	...
	private static class FeatureRegisterHandler {
		...
		public void registerFeature(RegistryEvent.Register<Feature<?>> event) {
			CUSTOM_MATCH = Registry.register(Registry.RULE_TEST, new ResourceLocation("earth_pulsation:nickel_ore_match"),
			...
			event.getRegistry().register(feature.setRegistryName("nickel_ore"));
			Registry.register(WorldGenRegistries.CONFIGURED_FEATURE, new ResourceLocation("earth_pulsation:nickel_ore"), configuredFeature);
		}
	}
	@SubscribeEvent
	public void addFeatureToBiomes(BiomeLoadingEvent event) {
		boolean biomeCriteria = false;
		if (new ResourceLocation("earth_pulsation:blazing_wasteland").equals(event.getName()))
			biomeCriteria = true;
		if (!biomeCriteria)
			return;
