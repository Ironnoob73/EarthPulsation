...
package idv.hailelectronic.earthpulsation.world.features;
...
import idv.hailelectronic.earthpulsation.procedures.ObeliskIgnoreBiomeProcedure;

public class ObeliskPointFeature extends Feature<NoneFeatureConfiguration> {
	public static final ObeliskPointFeature FEATURE = (ObeliskPointFeature) new ObeliskPointFeature()
			.setRegistryName("earth_pulsation:obelisk_point");
	public static final ConfiguredFeature<?, ?> CONFIGURED_FEATURE = FEATURE.configured(FeatureConfiguration.NONE);
	public static final Set<ResourceLocation> GENERATE_BIOMES = null;
	private StructureTemplate template = null;

	public ObeliskPointFeature() {
	...
	public boolean place(FeaturePlaceContext<NoneFeatureConfiguration> context) {
		...
		if (!dimensionCriteria)
			return false;
		if (template == null)
			template = context.level().getLevel().getStructureManager().getOrCreate(new ResourceLocation("earth_pulsation", "obelisk_point"));
		if (template == null)
			return false;
		if ((context.random().nextInt(1000000) + 1) <= 10000) {
			...
			for (int a = 0; a < count; a++) {
				...
				int y = spawnTo.getY();
				int z = spawnTo.getZ();
				if (!ObeliskIgnoreBiomeProcedure.execute(world, x, y, z))
					continue;
