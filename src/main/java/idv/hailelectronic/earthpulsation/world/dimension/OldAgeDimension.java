...
public class OldAgeDimension {
	@Mod.EventBusSubscriber(bus = Mod.EventBusSubscriber.Bus.MOD)
	public static class Fixers {
		@SubscribeEvent
		public static void registerFillerBlocks(FMLCommonSetupEvent event) {
			Set<Block> replaceableBlocks = new HashSet<>();
			replaceableBlocks.add(EarthPulsationModBlocks.YELLOW_STONE);
			...
			replaceableBlocks.add(ForgeRegistries.BIOMES.getValue(new ResourceLocation("earth_pulsation:ice_desert")).getGenerationSettings()
					.getSurfaceBuilder().get().config().getTopMaterial().getBlock());
			replaceableBlocks.add(ForgeRegistries.BIOMES.getValue(new ResourceLocation("earth_pulsation:ice_desert")).getGenerationSettings()
					.getSurfaceBuilder().get().config().getUnderMaterial().getBlock());
			replaceableBlocks.add(ForgeRegistries.BIOMES.getValue(new ResourceLocation("earth_pulsation:blazing_wasteland")).getGenerationSettings()
					.getSurfaceBuilder().get().config().getTopMaterial().getBlock());
			replaceableBlocks.add(ForgeRegistries.BIOMES.getValue(new ResourceLocation("earth_pulsation:blazing_wasteland")).getGenerationSettings()
					.getSurfaceBuilder().get().config().getUnderMaterial().getBlock());
			event.enqueueWork(() -> {
		...
		public static void registerDimensionSpecialEffects(FMLClientSetupEvent event) {
			...
			event.enqueueWork(() -> DimensionSpecialEffects.EFFECTS.put(new ResourceLocation("earth_pulsation:old_age"), customEffect));
		}
	}
}
